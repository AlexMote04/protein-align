...
{
  extern __shared__ unsigned char shared_query[];
  int gindex = blockIdx.x * blockDim.x + threadIdx.x;

  for (int i = threadIdx.x; i < NUM_ROWS - 1; i += blockDim.x)
  {
    shared_query[i] = query_seq[i];
  }
  __syncthreads();
  ...
  if (gindex < NUM_COLS_GLOBAL - NUM_RESIDUES)
  {
    ...
    for (int i = 1; i < NUM_ROWS; i++)
    {
      ...
      for (int j = 1; j < NUM_COLUMNS_LOCAL; j++)
      {
        ...
        int16_t val = H[diag] + cuda_blosum62[shared_query[i - 1] * 24 + db_residues[db_residue_idx]];
...
int swGPU(std::vector<int> &scores, const std::vector<unsigned char> &query_seq, const std::vector<unsigned char> &db_residues, const std::vector<int> &db_offsets)
{
  ...
  align<<<BLOCKS, THREADS, query_seq_bytes>>>(d_scores, d_query_seq, d_db_residues, d_db_offsets, d_H, d_E, d_F, NUM_ROWS, NUM_COLS, NUM_RESIDUES);
