...
int align_sw(const unsigned char *query_seq, const unsigned char *target_seq, const int M, const int N, std::vector<int32_t> &H, std::vector<int32_t> &E, std::vector<int32_t> &F)
{
  int32_t max_score = 0; // Keep track of optimal alignment score
  ...
  { // Skip first row
    ...
    { // Skip first column
      ...
      int32_t val = H[idx - N - 1] + blosum62[query_seq[i - 1] * 24 + target_seq[j - 1]]; // Substitution

      // Find max of scores
      val = std::max(E[idx], val);
      val = std::max(F[idx], val);
      H[idx] = std::max(val, (int32_t)0);
...
int align_nw(const unsigned char *query_seq, const unsigned char *target_seq, const int M, const int N, std::vector<int32_t> &H, std::vector<int32_t> &E, std::vector<int32_t> &F)
{
  ...
  {
    H[j] = -OPEN - (j - 1) * EXTEND;
    F[j] = NEG_INF_32; // Vertical gap impossible here
  }
  ...
  for (int i = 1; i < M; i++)
  {
    ...
    E[idx] = NEG_INF_32; // Horizontal gap impossible here
  }
  ...
  for (int i = 1; i < M; i++)
  {
    for (int j = 1; j < N; j++)
    {
      ...
      int32_t val = H[idx - N - 1] + blosum62[query_seq[i - 1] * 24 + target_seq[j - 1]];
...
{
  int (*align)(const unsigned char *query_seq, const unsigned char *target_seq, const int M, const int N, std::vector<int32_t> &H, std::vector<int32_t> &E, std::vector<int32_t> &F);
  if (algorithm == 0)
  {
    align = &align_nw;
  }
  ...
  std::vector<int32_t> H(M * N_max);
  std::vector<int32_t> E(M * N_max);
  std::vector<int32_t> F(M * N_max);
