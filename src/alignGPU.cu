    ...
    const int NUM_RESIDUES)
{
    ...
    if (gindex < NUM_COLS_GLOBAL - NUM_RESIDUES)
    {
        ...
        for (int i = 1; i < NUM_ROWS; i++)
        {
            ...
            int16_t h_diag = H[prev_row * NUM_COLS_GLOBAL];

            // Initialize the first column for the CURRENT row
            H[curr_row * NUM_COLS_GLOBAL] = h_left;
            ...
            for (int j = 1; j < NUM_COLUMNS_LOCAL; j++)
            {
                ...
                // Only need to fetch these from global memory
                int16_t h_up = H[index_prev];
                int16_t f_up = F[index_prev];

                // Compute using our fast registers
                ...
                int16_t val = h_diag + cuda_blosum62[cuda_query_seq[i - 1] * 24 + db_residues[db_residue_idx]];
        ...
        scores[gindex] = H[final_row * NUM_COLS_GLOBAL + (NUM_COLUMNS_LOCAL - 1)];
    }
}
...
int alignGPU(const int algorithm, std::vector<int> &scores, const std::vector<unsigned char> &query_seq, const std::vector<unsigned char> &db_residues, const std::vector<int> &db_offsets)
{
    ...
    CUDA_CHECK(cudaMemcpyToSymbol(cuda_query_seq, query_seq.data(), query_seq_bytes));
    ...
    if (algorithm == 0)
        align_nw<<<BLOCKS, THREADS>>>(d_scores, d_db_residues, d_db_offsets, d_H, d_F, NUM_ROWS, NUM_COLS, NUM_RESIDUES);
    else
