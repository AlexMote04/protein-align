...
NUM_SEQS = 128_000  # larger than Swiss-Prot so all ~574k sequences are loaded
...
def run_benchmark(algo: str) -> dict[str, dict[str, float]]:
    ...
    # benchmark.cpp prints: parasail, cpu, gpu  (in that order)
    keys = ["parasail", "cpu", "gpu"]
    result = {}
...
OUTPUT_CSV = PROJECT_ROOT / "results" / "experiment_1.csv"
...
def write_csv(results: dict[str, dict[str, dict[str, float]]]) -> None:
    ...
    with OUTPUT_CSV.open("w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(
            ["Implementation", "Algorithm", "Latency (ms)", "GCUPS", "Speedup vs CPU"]
        )
        for algo in ALGORITHMS:
            cpu_ms = results[algo]["cpu"]["latency_ms"]
            for key in ROW_ORDER:
                r = results[algo][key]
                speedup = cpu_ms / r["latency_ms"]
                writer.writerow(
                    [
                        LABELS[key],
                        algo.upper(),
